...
case "$command" in
  "create" )
    ...
    if [ "$(docker container ls -a -f name=jrvs-psql | wc -l)" -eq 2 ]; then
      echo "The docker container jrvs-psql has already been created. Check it's usage below:"
    ...
    if [ -z $db_database ] || [ -z $db_password ]; then
      echo "Illegal number of parameters. Might be missing db_username and/or db_password. Correct usage: ./scripts/psql_docker.sh create [db_username][db_password]"
      exit 1
    fi
